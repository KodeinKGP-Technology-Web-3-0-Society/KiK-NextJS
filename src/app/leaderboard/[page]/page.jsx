        ...
        <div className="flex min-h-screen flex-col before:absolute before:inset-0 before:rounded-[4px] before:border-[3px] before:border-transparent before:[border-image-source:linear-gradient(108.74deg,rgba(33,138,203,0.6)_0%,rgba(255,255,255,0.54)_36.46%,rgba(255,255,255,0.3)_73.96%,rgba(17,227,251,0.6)_100%)] before:[border-image-slice:1] before:content-[''] before:pointer-events-none shadow-[0_0_50px_-25px_rgba(0,0,0,0.5)] backdrop-blur-[100px] bg-[linear-gradient(108.74deg,rgba(255,255,255,0.24)_0%,rgba(255,255,255,0.06)_100%)]">
            ...
            <ul className="space-y-1">
                {currentPageData.map((user) => (
                    <li key={user.rank} className="flex items-center space-x-4 px-4 ml-5 bg-gradient-to-r from-[rgba(17,227,251,0.3)] to-[rgba(255,255,255,0.06)] transition-transform duration-200 hover:scale-[1.01]">
                        <span className="text-lg w-6 text-right font-bold">{user.rank}.</span>
                        <img
                        ...
                        />
                        <span className="flex-1 bg-gradient-to-b from-[#24E8FF] to-[#0CC5DA] bg-clip-text text-transparent font-bold">{user.name}</span>
                        <span className="bg-gradient-to-b from-[#24E8FF] to-[#0CC5DA] bg-clip-text text-transparent font-bold">{user.score}</span>
                    </li>
                ))}
                ...
                <li className="flex items-center space-x-4 px-4 ml-5 mt-4 bg-[#11E3FB] text-black shadow-lg transition-transform duration-200 hover:scale-[1.01]">
                    <span className="text-lg w-6 text-right font-bold">{currentUser.rank}.</span>
                    <img
                        src={`https://robohash.org/${encodeURIComponent(currentUser.name)}?set=set4`}
                        alt={currentUser.name}
                        className="w-8 h-8 my-1 rounded-full object-cover border-2 border-white"
                    />
                    <span className="flex-1 font-bold">{currentUser.name}</span>
                    <span className="font-bold pr-4">{currentUser.score}</span>
                </li>
            </ul>
