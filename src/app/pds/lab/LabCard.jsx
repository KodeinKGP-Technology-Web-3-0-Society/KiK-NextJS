...
export default function LabCard({ qna, topic, subTopic, ind }) {
	...
	const displayText = shortWords.length > shortChars.length ? shortChars : shortWords

	return (
		<tr className="border border-[#555] bg-[#050a24] shadow-md transition-transform duration-200">
			<td className="text-white cursor-pointer px-2" onClick={toggleCompleted}>
				{isComp ? <CheckSquare size={20} /> : <Square size={20} fill={'white'} />}
			</td>
			<td className="py-2 pl-3 pr-2 align-middle text-[1.2rem] text-left w-full whitespace-nowrap">
				<Link
					href={path}
			...
			<td className="text-white cursor-pointer px-2" onClick={toggleFavourite}>
				<Star 
                size={24} 
